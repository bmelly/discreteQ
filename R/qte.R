...
qte <- function(y, d, x=NULL, w=NULL, q.range=c(0.05,0.95), method="logit", bsrep=200, alpha=0.05){
  y0 <- y[d==0]
  y1 <- y[d==1]
  n0 <- length(y0)
  n1 <- length(y1)
  ...
  QTE.func <- stepfun(knots.new, c(Q1.func(knots.new) - Q0.func(knots.new), max(ys)), right=TRUE)
  knots.new <- sort(unique(c(knots(lb.Q1j.func), knots(ub.Q0j.func))))
  lb.QTE.func <- stepfun(knots.new, c(lb.Q1j.func(knots.new) - ub.Q0j.func(knots.new), max(ys)), right=TRUE)
  knots.new <- sort(unique(c(knots(ub.Q1j.func), knots(lb.Q0j.func))))
  ub.QTE.func <- stepfun(knots.new, c(ub.Q1j.func(knots.new) - lb.Q0j.func(knots.new), max(ys)), right=TRUE)
...
#summary
summary.qte <- function(object, taus=0.05, which="QTE"){
  if(length(taus)==1){
...
#plot
plot.qte <- function(object, which=NULL, xlim=NULL, ylim=NULL, main=NULL, xlab=NULL, ylab=NULL, add=FALSE, col.l="dark blue", col.b="light blue", shift=NULL, lty.l=1, lwd.l=1, lty.b=1, lwd.b=5){
