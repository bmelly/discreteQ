...
decomposition <- function(y, x, g, w=NULL, q.range=c(0.05,0.95), method="logit", bsrep=200, alpha=0.05){
  x <- as.matrix(x)
  y0 <- y[g==0]
  y1 <- y[g==1]
  x0 <- x[g==0, , drop=FALSE]
  ...
  observed <- stepfun(knots.new, c(Q1.func(knots.new) - Q0.func(knots.new), max(ys)), right=TRUE)
  knots.new <- sort(unique(c(knots(lb.Q1j.func), knots(ub.Q0j.func))))
  lb.observed <- stepfun(knots.new, c(lb.Q1j.func(knots.new) - ub.Q0j.func(knots.new), max(ys)), right=TRUE)
  knots.new <- sort(unique(c(knots(ub.Q1j.func), knots(lb.Q0j.func))))
  ub.observed <- stepfun(knots.new, c(ub.Q1j.func(knots.new) - lb.Q0j.func(knots.new), max(ys)), right=TRUE)

  knots.new <- sort(unique(c(knots(Q1.func), knots(Qc.func))))
  unexplained <- stepfun(knots.new, c(Q1.func(knots.new) - Qc.func(knots.new), max(ys)), right=TRUE)
  knots.new <- sort(unique(c(knots(lb.Q1j.func), knots(ub.Qcj.func))))
  lb.unexplained <- stepfun(knots.new, c(lb.Q1j.func(knots.new) - ub.Qcj.func(knots.new), max(ys)), right=TRUE)
  knots.new <- sort(unique(c(knots(ub.Q1j.func), knots(lb.Qcj.func))))
  ub.unexplained <- stepfun(knots.new, c(ub.Q1j.func(knots.new) - lb.Qcj.func(knots.new), max(ys)), right=TRUE)

  knots.new <- sort(unique(c(knots(Qc.func), knots(Q0.func))))
  composition <- stepfun(knots.new, c(Qc.func(knots.new) - Q0.func(knots.new), max(ys)), right=TRUE)
  knots.new <- sort(unique(c(knots(lb.Qcj.func), knots(ub.Q0j.func))))
  lb.composition <- stepfun(knots.new, c(lb.Qcj.func(knots.new) - ub.Q0j.func(knots.new), max(ys)), right=TRUE)
  knots.new <- sort(unique(c(knots(ub.Qcj.func), knots(lb.Q0j.func))))
  ub.composition <- stepfun(knots.new, c(ub.Qcj.func(knots.new) - lb.Q0j.func(knots.new), max(ys)), right=TRUE)
...
#summary
summary.decomposition <- function(object, taus=0.05, which="unexplained"){
...
#plot
plot.decomposition <- function(object, which="decomposition", xlim=NULL, ylim=NULL, main=NULL, xlab=NULL, ylab=NULL, add=FALSE, col.l="dark blue", col.b="light blue", shift=NULL, lty.l=1, lwd.l=1, lty.b=1, lwd.b=5){
